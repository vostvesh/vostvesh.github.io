/*
  inspired from: https://dribbble.com/shots/2135617-Happy-4th-Of-July2
*/
...
const fireWorkColors = ['#FF490E', '#FF9D1F', '#FF0D4B', '#FFF587', '#F0F0F2'];
...
class FireWork {
  ...
  reset() {
    ...
    this.particleLength = (canvas.width <= 500)? randomIntFromRange(5,13) : randomIntFromRange(8, 20);
...
class Building {
  ...
  draw() {
    ...
    // windows
    const paddingV = this.height*0.15, paddingH = this.width*0.15;
    const gapV = this.height*0.1, gapH = this.width*0.1;
    let windows = [];
    
    if(this.windowType === 1) {
      const WINDOW_AMOUNT = 5;
      const windowW = this.width - paddingH*2;
      const windowH = (this.height - paddingV*2 - gapV*(WINDOW_AMOUNT - 1)) / WINDOW_AMOUNT;
      ...
      const windowW = (this.width - paddingH*2 - gapH*(WINDOW_AMOUNT - 1)) / WINDOW_AMOUNT;
...
function init() {
  ...
  let currX = -20;
  const minW = 25, maxW = 40;
  const minH = 25, maxH = 50;
  ...
  while (currX <= canvas.width) {
    ...
    buildings.push(
      new Building(currX, canvas.height - height, width, height, randomIntFromRange(0,2))
    );
